        ...
        evernote.listNotebooks(edamUser, function(err, notebookList) {
          ...
          evernote.findNotes(edamUser,  '', { tagGuids : guid_list }, function(err, noteList) {
            ...
            if (guid_list.length > 0 && noteList.notes.length > 0) {
              callback({name:'readerCreateNoteException'}, 304); 
            } else {
...
//  str += '<div>' + description.replace(/<div/g, '<span').replace(/div>/g, 'span>') + '</div>';
  str += '<div>' + description.replace(/class=/g, 'data-class=')+ '</div>';
...
/*
    ...
    str += '</style>';
*/
    str += '<div><a style="text-decoration:none;"  href="http://for.evereader.io">FOR.EVEREADER.IO</a></div>';
    str += '</div>';
    return str;
}
