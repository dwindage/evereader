    ...
    for(var i=0; i<keys.length; i++) {
        if( prohibited_attributes.indexOf(keys[i]) < 0 ) { // not found
            if ( /[a-zA-Z0-9]+/.test(keys[i]) ) { // attribute name must be english
                ...
                if ( permitted_attributes.indexOf(keys[i]) < 0 ) {
                    foundInvalidateValues = true;
                }

                // remove javascripts
                if( foundInvalidateValues == false && /^on.*/.test(keys[i]) == false) {
                    resultAttribute += keys[i] + '="' + attr[keys[i]].replace(/&amp;/g, '&').replace(/&/g, '&amp;') + '" ';
                }
            }
        }
    }
...
//            resultHtml += dom[i].data.replace(/&amp;/g, '&').replace(/&/g, '&amp;');
            resultHtml += dom[i].data;
        }
    }
    return resultHtml;
};
