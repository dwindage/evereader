...
function get_rss_feed_data(message, outputQueue) {
    ...
    message.rss_url_list.forEach(function(rss_url) {
        evereader.parse(rss_url, function(err, article_list) {
            var out_message = message.clone();
            out_message.rss_url_list = [];
            out_message.rss_url_list.push(rss_url);
            if(err) {
                console.log('url', rss_url, 'error', err);
                out_message.error = err;
                return;
            }
...
//    if(message.error) return;
	...
	console.log(message.user_token, message.rss_url_list, message.status_code, message.error); 
}
